...
java 面向对象之继承总结:
    ...
    5.this 与 super 可以很好的分清子类与父类
    6.继承可以添加额外的属性与方法(多态的呈现 大概是这样来的把)
    7.子类创建对象时父类会被初始化是因为子类的构造方法默认带 super() (java所有的类都默认带有)
    8. 父类带有有参构造方法而没有无参构造方法时,子类构造方法必须初始化父类的实例变量
    9.子类继承父类的所有非私有成员,但不包括构造方法 但子类可以通过 super()初始化父类;
    10. 每个类都默认有一个默认无参构造方法 ,当自己写了构造方法后 这个默认无参构造方法不再生效
目前暂时就这么多
 */
